...
float lux::imagePlaneValue( float v, float dG, float dB )
{
   if( v == 0.0 ){ return 0.0; }
   return pow(v, dG) * dB;
}
...
void lux::writeOIIOImage( const char* fname, Image& img, float displayBrightness, float displayGamma  )
{
   ...
   for( int j=0;j<img.Height();j++ )
   {
      for( int i=0;i<img.Width();i++ )
      {
	      ...
	      for( size_t c=0;c<(size_t)img.Depth();c++ )
         {
            if(c < 3)
            {
               pix[c] = lux::imagePlaneValue( pix[c], displayGamma, displayBrightness );
            }
   ...
   lux::writeOIIOImage(fname, imagedata, img.Width(), img.Height(), img.Depth());

   delete [] imagedata;

}

void lux::writeOIIOImage( const char* fname, const float* imagedata, const int width, const int height, const int depth )
{
   ...
   else
   {
      ...
      if( depth > 0 ){ spec.channelnames.push_back ("R"); }
      if( depth > 1 ){ spec.channelnames.push_back ("G"); }
      if( depth > 2 ){ spec.channelnames.push_back ("B"); }
      if( depth > 3 ){ spec.channelnames.push_back ("A"); spec.alpha_channel = 3; }
      if( depth > 4 ){ spec.channelnames.push_back ("Z"); spec.z_channel = 4; }
      ...
      datetime = 
         oiiotostr(tm.tm_year + 1900) + "-" + 
         oiiotostr(tm.tm_mon + 1) + "-" + 
         oiiotostr(tm.tm_mday) + " " + 
         oiiotostr(tm.tm_hour) + ":" +  
         oiiotostr(tm.tm_min) + ":" + 
         oiiotostr(tm.tm_sec);
...
void lux::writeOIIOImage( const char* fname, Image& img, const map<string,string>& labels, float displayBrightness, float displayGamma )
{
   ...
   for( int j=0;j<img.Height();j++ )
   {
	 ...
	 for( size_t c=0;c<(size_t)img.Depth();c++ )
         {
            if(c<3){ pix[c] = lux::imagePlaneValue( pix[c], displayGamma, displayBrightness ); }
	    imagedata[index++] = pix[c];
         }
      }
   }
